  ...
  it('persists normalized items and a private snapshot, then deduplicates and records failure', async () => {
    ...
    const feed = `<?xml version="1.0"?>
      <rss version="2.0"><channel><item>
        ...
        <description><![CDATA[This official update describes production architecture, evaluation, monitoring, security controls, testing, deployment, incident response, and practical lessons for teams building reliable AI agents.]]></description>
      </item></channel></rss>`;
    ...
    expect(third).toMatchObject({ status: 'failed', errorCode: 'invalid_xml' });
    expect(database.prepare(`
      SELECT status, error_code FROM source_fetch_run
      WHERE id = ?
    `).get(third.runId)).toEqual({ status: 'failed', error_code: 'invalid_xml' });
    expect(database.prepare(`
