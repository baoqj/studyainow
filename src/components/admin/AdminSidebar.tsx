...
const navItems = [
  { label: '概览', icon: LayoutDashboard, path: '/admin' },
  ...
  return (
    <>
      ...
      <aside className={`admin-sidebar-surface fixed inset-y-0 left-0 z-50 flex w-[252px] flex-col border-r border-blue-300/25 shadow-xl transition-transform duration-200 lg:translate-x-0 lg:shadow-none ${open ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="px-5 pb-2 pt-5 text-[10px] font-bold uppercase tracking-[0.18em] text-blue-200/70">Admin console</div>
        <nav className="flex-1 space-y-1 overflow-y-auto px-3 py-2" aria-label="管理员主菜单">
          {navItems.map((item) => (
            <NavLink
              ...
              to={item.path}
              onClick={onClose}
              className={({ isActive }) => `group flex items-center gap-3 rounded-md px-3 py-2.5 text-sm transition-colors ${isActive ? 'bg-blue-100 font-semibold text-[#123a5f]' : 'text-blue-100/85 hover:bg-blue-400/20 hover:text-white'}`}
            >
