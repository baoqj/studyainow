...
type Section = 'overview' | 'sources' | 'candidates' | 'articles' | 'taxonomy';
...
export function AdminNews() {
  ...
  const loadLists = useCallback(async () => {
    ...
    try {
      const [dashboard, sourcePayload, taxonomyPayload, candidatePayload, articlePayload] = await Promise.all([
...
function StoryPanel({ story, taxonomy, busy, back, refresh, generate, review, createDraft }: { story: StoryResearch; taxonomy: TaxonomyNode[]; busy: boolean; back: () => void; refresh: () => Promise<void>; generate: () => void; review: (claim: Claim, status: string) => void; createDraft: () => void }) {
